...
# Adding Arguments
parser.add_argument('--image', type = str, help ='give path to input video')  
...
out = cv2.VideoWriter('temp/temp_video.mp4',cv2.VideoWriter_fourcc(*'mp4v'), 15, size)
for i in range(3):
  out.write(img)
out.release()

source = "temp/temp_video.mp4"
...
retina_face(global_people_info)

face_sr(global_people_info)
...
if args.visual:
    ...
    for i in range(len(global_people_info)):
        ...
        x1 = int(df["bb_xmin"][i])
        y1 = int(df["bb_ymin"][i])
        x2 = int(df["bb_xmin"][i] + df["bb_width"][i])
        y2 = int(df["bb_ymin"][i] + df["bb_height"][i])

        temp_frame = cv2.rectangle(temp_frame, (x1, y1), (x2, y2), (255,0,0), 2)
        font = cv2.FONT_HERSHEY_TRIPLEX
        if global_people_info[i].age != -1:
            temp_frame = cv2.putText(temp_frame, 'L{} {}{}'.format(global_people_info[i].id, global_people_info[i].age, global_people_info[i].gender), (x1,y1+15), font, 0.5, (255, 0, 0), 1, cv2.LINE_AA)
